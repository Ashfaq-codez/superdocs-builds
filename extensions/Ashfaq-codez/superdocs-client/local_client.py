...
class LocalSuperDocsClient(SuperDocsClientInterface):
    ...
    def __init__(self, runtime_dir: Path):
        ...
        # In-memory tracking for the local runtime
    ...
    async def export(self, document: DocumentRef, formats: List[ExportFormat]) -> ExportResult:
        ...
        # Generate Real DOCX with professional formatting
        ...
        # Generate Real PDF with professional formatting
        if ExportFormat.PDF in formats:
            ...
            pdf.cell(0, 10, "SuperDocs Inc.", ln=True, align="L")
            ...
            for line in safe_text.split('\n'):
                ...
                if clean_line.startswith('# '):
                    ...
                    pdf.cell(0, 8, text=clean_line[2:], ln=True)
                    pdf.ln(2)
                elif clean_line.startswith('## '):
                    pdf.set_font("Helvetica", style="B", size=12)
                    pdf.cell(0, 6, text=clean_line[3:], ln=True)
                    pdf.ln(2)
                else:
                    pdf.set_font("Helvetica", size=10)
        ...
        # 2. Setup isolated artifact directory
        export_dir = self.runtime_dir / doc_id
        export_dir.mkdir(parents=True, exist_ok=True)
        artifacts = []

        # 3. Generate Real DOCX
        if ExportFormat.DOCX in formats:
            docx_path = export_dir / "offer_letter.docx"
            doc = Document()
            for line in approved_text.split('\n'):
            ...
            # FIX: Convert pathlib.Path to string for python-docx
            doc.save(str(docx_path))
            artifacts.append(ExportArtifact(format=ExportFormat.DOCX, reference=f"{doc_id}/offer_letter.docx"))
        ...
        if ExportFormat.PDF in formats:
            ...
            # fpdf2 requires latin-1 safe encoding for standard fonts
            safe_text = approved_text.encode('latin-1', 'replace').decode('latin-1')
            for line in safe_text.split('\n'):
                # FIX: Use 'text' parameter instead of 'txt' for fpdf2 compatibility
